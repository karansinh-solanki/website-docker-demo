pipeline {  
    ...
    environment {  
        ...
        AWS_ACCOUNT_ID = '314076239426'  
        IMAGE_TAG = "${env.BUILD_NUMBER}"  
        IMAGE_URI = "${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/${ECR_REPO}:${IMAGE_TAG}"  
        LATEST_URI = "${AWS_ACCOUNT_ID}.dkr.ecr.${AWS_REGION}.amazonaws.com/${ECR_REPO}:latest"  
        DEPLOY_SERVER = '15.206.73.34'  
    }  
  
    stages {  
        stage('Checkout') {  
            steps {  
                git branch: 'master', url: 'https://github.com/karansinh-solanki/website-docker-demo.git'  
            }  
        }  
        ...
        stage('Deploy to EC2') {  
            steps {  
                sshagent(['deploy-ec2-key']) {  
                    sh '''  
                        ssh -o StrictHostKeyChecking=no ubuntu-user@$DEPLOY_SERVER "  
